-- A multi-release remains a critical hardware incident, but it no longer blocks
-- the entire kiosk when the paid/selected slot and battery are explicitly among
-- the observed released identities. The selected customer rental may continue;
-- no second eject command is allowed.
create or replace function public.append_rental_orchestrator_event(
  ...
  p_station_id text default null::text,
  p_battery_id text default null::text,
  p_final_amount_chf numeric default null::numeric,

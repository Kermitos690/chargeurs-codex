-- PassStudio's loyalty template has no custom per-member credit field, but it
-- does render its per-member "tier" field on both Apple and Google Wallet.
-- Keep the actual rental-credit balance in the ledger and project a concise,
-- currency-qualified display into that supported field.  No balance, payment,
-- rental, or hardware state is changed here.
...
begin
  ...
  v_wallet_tier := left(
    'Crédit location : ' || v_credit_display ||
    case when nullif(v_tier, '') is null then '' else ' · ' || v_tier end,
    80
  );
...
-- The prior credit event was delivered before the template-supported field was
-- selected. Queue one idempotent refresh for each active PassStudio pass.
do $block$
declare
  pass_row record;
begin
  ...
  loop
    perform public.enqueue_customer_wallet_sync_event(
      pass_row.user_id,
      'membership_credit_visible_in_level',
      'membership-credit-visible-v2:' || pass_row.id::text,
      null,
      jsonb_build_object('reason', 'membership_credit_visible_in_level'),
      now() + interval '1 day'
    );
    ...
    insert into public.customer_wallet_native_notifications(
      ...
      'membership_credit_visible_in_level',
      'membership-credit-visible-v2:' || pass_row.id::text,
      'Crédit location visible',
      'Votre Chargeurs+ Pass affiche maintenant votre crédit de location.',
      jsonb_build_object('reason', 'membership_credit_visible_in_level'),
      now() + interval '1 day'

-- Test-only self-enrollment for the diagnostic APK.
-- It is deliberately limited to stations explicitly marked as staging pilots.
-- Production devices continue to require one-time pairing codes.
...
begin
  ...
  -- A reinstall can produce a new local device id. Retire only older diagnostic
  -- devices on this staging pilot; never touch release/production devices.

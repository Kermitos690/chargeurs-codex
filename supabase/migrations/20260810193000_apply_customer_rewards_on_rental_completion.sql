...
begin
  ...
  select customer_user_id,
         coalesce(final_amount_cents, round(coalesce(new.final_amount_chf,0) * 100)::bigint) / 100.0
  ...
  for v_rule in
    select * from public.customer_chargepoints_rules
    where active = true
  ...
  loop
    ...
    if v_points > 0 then
      insert into public.customer_chargepoints_ledger(
        ...
        v_user_id, v_points, 'rental_completed', 'rental', new.rental_id::text,
        'chargepoints:rule:' || v_rule.id::text || ':rental:' || new.rental_id::text,
        jsonb_build_object('rule_id',v_rule.id,'rule_code',v_rule.code,'final_amount_chf',coalesce(v_final_chf,0),'orchestrator_event_id',new.id)
...
drop trigger if exists trg_apply_customer_rewards_on_rental_completion on public.rental_orchestrator_events;
create trigger trg_apply_customer_rewards_on_rental_completion
after insert on public.rental_orchestrator_events
for each row when (new.event_type = 'rental_completed')
execute function public.apply_customer_rewards_on_rental_completion();

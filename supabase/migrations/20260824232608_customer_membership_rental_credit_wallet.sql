...
begin
  select customer_user_id, membership_credit_applied_cents, membership_credit_reversed_cents, membership_credit_committed_cents
    into v_user_id, v_applied, v_reversed, v_committed
  from public.rental_sessions
  where id = p_rental_id
  for update;
  ...
  insert into public.customer_membership_credit_ledger(
    ...
    'rental_settlement_committed', v_key, jsonb_build_object('reserved_cents', v_open)
  ) on conflict (idempotency_key) do nothing;
  update public.rental_sessions
  set membership_credit_committed_cents = membership_credit_committed_cents + v_open,
...
declare
  ...
  v_committed bigint;
  v_open bigint;
  v_key text := format('membership_credit_reversal:%s', p_rental_id);
begin
  select customer_user_id, membership_credit_applied_cents, membership_credit_reversed_cents, membership_credit_committed_cents
    into v_user_id, v_applied, v_reversed, v_committed
  from public.rental_sessions
  ...
  insert into public.customer_membership_credit_ledger(
    ...
    'CHF', p_reason, v_key, jsonb_build_object('reversal_of', 'rental_settlement')
  ) on conflict (idempotency_key) do nothing;
  update public.rental_sessions
...
revoke all on function public.commit_customer_membership_credit_for_rental(uuid) from public, anon, authenticated;
...
grant execute on function public.commit_customer_membership_credit_for_rental(uuid) to service_role;

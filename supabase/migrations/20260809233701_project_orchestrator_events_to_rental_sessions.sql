...
begin
  if new.event_type = 'battery_released' then
    ...
    update public.rental_sessions set started_at = coalesce(started_at, new.occurred_at), updated_at = now() where id = new.rental_id;
...
drop trigger if exists trg_project_orchestrator_event_to_rental_session on public.rental_orchestrator_events;
create trigger trg_project_orchestrator_event_to_rental_session
after insert on public.rental_orchestrator_events
for each row execute function public.project_orchestrator_event_to_rental_session();
...
set ejected_at = coalesce(rs.ejected_at, e.occurred_at),
    state = case when public.rental_session_state_rank(rs.state) < public.rental_session_state_rank('ejected') then 'ejected' else rs.state end,
...
update public.rental_sessions rs
set state = case when public.rental_session_state_rank(rs.state) < public.rental_session_state_rank('battery_returned') then 'battery_returned' else rs.state end,

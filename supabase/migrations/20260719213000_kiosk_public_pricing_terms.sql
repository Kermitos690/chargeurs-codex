-- The kiosk must disclose the real initial debit/authorization and the public
-- billing rules before opening Stripe Checkout. Only non-sensitive pricing
-- fields are returned, after station-bound kiosk authentication.
...
comment on function public.kiosk_quote(text, text) is
  'Station-bound kiosk quote including the public deposit/rate/cap/non-return terms; never returns internal assignment data.';

-- P0 field fix: the beta release trigger still enforced the retired 75ct/30m
-- guest profile and rejected the authoritative tiered Premium profile before a
-- rental session could be inserted. Keep the gate strict, but validate the
-- server-resolved canonical Premium guest shape instead of weakening safety.
...
BEGIN
  ...
  IF v_segment = 'guest' THEN
    ...
    IF coalesce(v_profile.period_minutes, -1) <> 1440
       ...
       OR coalesce(v_profile.unreturned_fee_cents, -1) <> 620

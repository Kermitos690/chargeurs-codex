-- Absolute DB-side hardware quarantine. This protects even an older deployed
-- eject-after-payment bundle: no ChargeNow release call can be reached after a
-- release_requested/test_ejection_resumed transition is refused here.
...
begin
  ...
  if p_event_type in ('release_requested', 'test_ejection_resumed')
     and exists (
       select 1 from public.station_hardware_quarantines q
       where q.station_id = v_effective_station and q.active = true
     ) then
  ...
  if p_event_type in ('battery_released', 'rental_activated')
     and not exists (
       ...
       where h.rental_session_id = p_rental_id and h.result = 'single_release'
     ) then
...
grant execute on function public.append_rental_orchestrator_event(
  uuid,bigint,text,text,timestamptz,jsonb,text,text,text,text,numeric,text
) to service_role;

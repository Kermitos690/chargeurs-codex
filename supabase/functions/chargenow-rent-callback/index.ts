...
function outIdentity(payload: unknown) {
  const row = flatten(payload);
  return {
    tradeNo: firstString(row, ["rentOrderId", "tradeNo", "trade_no", "orderNo", "orderId"]) ?? "",
    batteryId: firstString(row, ["outBatteryId", "batteryId", "batterySN", "batterySn"]) ?? "",
    slotNum: firstInt(row, ["outSlot", "slotNum", "slot", "slotId"]),
  };
}

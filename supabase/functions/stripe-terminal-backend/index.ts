...
async function projectState(db: any, stripe: Stripe, session: any, reconcile: boolean) {
  ...
  if (reconcile && intentId) {
    intent = await stripe.paymentIntents.retrieve(String(intentId));
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Reader-connectivity plane: no rental is created or required merely to connect the reader.
    ...
    // Restart/retry: an active existing PI remains authoritative and is reused.
    ...
    // If Stripe may have accepted a create/cancel request, never release the rail automatically.

...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (["ejected", "battery_taken", "active_rental", "battery_returned", "closed", "completed"].includes(session.state)) {
      return ok({ ok: true, alreadyDone: true, batteryId: session.battery_id ?? null });
    }
    ...
    const { data: locked } = await db.from("rental_sessions")
      ...
      .select();
    if (!locked || locked.length === 0) return ok({ ok: true, alreadyInProgress: true });
    ...
    const res = await ejectByRent(cabinetId, requestedSlotNum, tradeNo ?? undefined);
    await logApi(db, {
      service: "chargenow", endpoint: "/cabinet/ejectByRent", method: "POST",
      status_code: res.status, request: { cabinetId, slotNum: requestedSlotNum }, response: res.data, error: res.error,
    });

    if (res.ok) {
      const released = extractEjectedBattery(res.data);
      ...
      return ok({ ok: true, slotNum: selectedSlotNum, batteryId: released.batteryId });
    }

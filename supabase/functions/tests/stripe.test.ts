...
Deno.test("webhook signature: missing signature rejected", async () => {
  let threw = false;
  try { await stripe.webhooks.constructEventAsync(evt(), "", WHSEC); } catch { threw = true; }
  assert(threw);
});
...
Deno.test("webhook signature: wrong secret rejected", async () => {
  ...
  let threw = false;
  try { await stripe.webhooks.constructEventAsync(payload, header, WHSEC); } catch { threw = true; }
  assert(threw);
});
...
Deno.test("payment match: exact amount + currency + snapshot => ok", () => {
  const r = evaluatePaymentMatch({
    expectedCents: 500, paidCents: 500, expectedCurrency: "CHF", paidCurrency: "chf",
    hasSnapshot: true, storedHash: "h", recomputedHash: "h", metaHash: "h",
  });
...
Deno.test("payment match: client-tampered amount => AMOUNT_MISMATCH, not ok", () => {
  const r = evaluatePaymentMatch({
    expectedCents: 500, paidCents: 100, expectedCurrency: "CHF", paidCurrency: "chf",
    hasSnapshot: false, storedHash: null, recomputedHash: null, metaHash: null,
  });
...
Deno.test("payment match: wrong currency => not ok", () => {
  const r = evaluatePaymentMatch({
    expectedCents: 500, paidCents: 500, expectedCurrency: "CHF", paidCurrency: "eur",
    hasSnapshot: false, storedHash: null, recomputedHash: null, metaHash: null,
  });
...
Deno.test("payment match: snapshot hash mismatch => SNAPSHOT_MISMATCH (priority)", () => {
  const r = evaluatePaymentMatch({
    expectedCents: 500, paidCents: 500, expectedCurrency: "CHF", paidCurrency: "chf",
    hasSnapshot: true, storedHash: "h", recomputedHash: "DIFFERENT", metaHash: "h",
  });
...
Deno.test("payment match: metadata hash spoofed but recomputed==stored still checks meta", () => {
  const r = evaluatePaymentMatch({
    expectedCents: 500, paidCents: 500, expectedCurrency: "CHF", paidCurrency: "chf",
    hasSnapshot: true, storedHash: "h", recomputedHash: "h", metaHash: "SPOOF",
  });
  assertEquals(r.snapshotOk, false);
});
...
Deno.test("refund: full when requested<=0; integer cents", () => {
  const r = evaluateRefund({ capturedCents: 500, requestedCents: 0, alreadyRefundedCents: 0 });
  assertEquals(r.ok, true);
  assertEquals(r.refundCents, 500);
});

Deno.test("refund: cannot exceed captured", () => {
  const r = evaluateRefund({ capturedCents: 500, requestedCents: 600, alreadyRefundedCents: 0 });
  assertEquals(r.ok, false);
  assertEquals(r.error, "EXCEEDS_CAPTURED");
});

Deno.test("refund: already fully refunded => no-op", () => {
  const r = evaluateRefund({ capturedCents: 500, requestedCents: 0, alreadyRefundedCents: 500 });
  assertEquals(r.ok, false);
  assertEquals(r.error, "ALREADY_REFUNDED");
});

Deno.test("refund: nothing captured", () => {
  const r = evaluateRefund({ capturedCents: 0, requestedCents: 100, alreadyRefundedCents: 0 });
  assertEquals(r.error, "NOTHING_CAPTURED");
});

Deno.test("refund: partial within remaining", () => {
  const r = evaluateRefund({ capturedCents: 500, requestedCents: 200, alreadyRefundedCents: 100 });
  assertEquals(r.ok, true);

...
Deno.serve(async (req) => {
  ...
  const active = (currentMemberships ?? []).find((membership) => {
    if (membership.status !== "active") return false;
    return !membership.ends_at || Date.parse(membership.ends_at) > Date.now();
  });
  if (active) {
    return json({
      ok: true,
      ...
      redirectUrl: `${origin}/compte/pass`,
    });
  }
  ...
  if (pending?.stripe_checkout_session_id) {
    try {
      ...
      if (existing.status === "open" && existing.url) {
        return json({ ok: true, checkoutUrl: existing.url, membershipId: pending.id, reused: true });
      }
    } catch (_) {
      // The previous Checkout can no longer be reused; create a new one below.
    }
  }
  ...
  if (!pending) {
    const { data: created, error: createError } = await db.from("customer_memberships")
      ...
      .select("id,plan_id,status,stripe_checkout_session_id,stripe_subscription_id,stripe_customer_id")
      .single();
  ...
  const metadata = {
    ...
    membership_id: String(pending.id),
    membership_plan_id: String(plan.id),
    membership_plan_code: String(plan.code),
  ...
  try {
    const checkout = await stripe.checkout.sessions.create({
      ...
      line_items: [{
        price_data: {
          currency: String(plan.currency || "CHF").toLowerCase(),
          unit_amount: feeCents,
          product_data: {
            name: `Chargeurs+ — ${plan.name}`,
            description: "Adhésion Client Chargeurs. Les tarifs de location et avantages applicables sont lus depuis le plan actif.",
          },
    ...
    const { error: updateError } = await db.from("customer_memberships").update({
    ...
    await auditLog(db, {
      actor: user.id,
      action: "membership.checkout_created",
      target: String(pending.id),
      data: { plan_code: plan.code, checkout_session_id: checkout.id },
    });

    return json({ ok: true, checkoutUrl: checkout.url, membershipId: pending.id });
  } catch (error) {
    const stripeCode = typeof (error as { code?: unknown })?.code === "string"
      ? String((error as { code: string }).code).toUpperCase()
      : "CHECKOUT_FAILED";
    await auditLog(db, {
      actor: user.id,
      action: "membership.checkout_failed",
      target: String(pending.id),
      data: { code: stripeCode },
    });

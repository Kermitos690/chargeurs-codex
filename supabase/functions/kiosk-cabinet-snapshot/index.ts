// kiosk-cabinet-snapshot — station-bound and strictly read-only.
// Raw supplier fields remain server-side; the kiosk only receives safe display values.
// Physical release reconciliation is intentionally owned by reconcile-pending-ejection,
// which compares the full four-slot pre/post hardware snapshots. This endpoint must
// never promote a rental to released/active from a single empty-slot observation.

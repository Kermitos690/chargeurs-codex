...
async function resolveClientContext(
  ...
  const baseQuery = db.from("rental_sessions")
    .select("id,station_id,state,created_at")
    .eq("customer_user_id", userId);
  const result = rentalId
    ? await baseQuery.eq("id", rentalId).maybeSingle()
    : await baseQuery.order("created_at", { ascending: false }).limit(1).maybeSingle();
  if (result.error) return { ok: false, code: "ACCOUNT_CONTEXT_UNAVAILABLE" };
  if (rentalId && !result.data) return { ok: false, code: "RENTAL_NOT_ACCESSIBLE" };

  const station = result.data?.station_id ? String(result.data.station_id) : normalizeStation(requestedStationId);
  return { ok: true, context: { rentalId: result.data?.id ? String(result.data.id) : null, stationId: station, rentalState: result.data?.state ? String(result.data.state) : null } };
}

async function rateLimited(db: ReturnType<typeof adminClient>, ipHash: string): Promise<boolean> {
  const since = new Date(Date.now() - 60 * 60 * 1000).toISOString();
  const { count } = await db.from("public_contact_requests").select("id", { head: true, count: "exact" }).eq("ip_hash", ipHash).gte("created_at", since);
  return (count ?? 0) >= 5;
}

async function notifySuperAdmins(db: ReturnType<typeof adminClient>, requestId: string, triage: ReturnType<typeof triageVoltMessage>, context: VoltServerContext) {
  try {
    ...
    await db.from("notifications").insert(userIds.map((userId) => ({
      ...
      data: { requestId, path: "/admin/support", category: triage.category, priority: triage.priority, stationId: context.stationId ?? null },
      status: "sent",
      sent_at: now,
    ...
    // The support case remains authoritative even when an admin alert cannot be recorded.
  }
}
...
Deno.serve(async (req) => {
  ...
  if (Number(req.headers.get("Content-Length") ?? 0) > 16_384) return response(origin, { ok: false, error: "PAYLOAD_TOO_LARGE" }, 413);
  ...
  if (action === "volt_case") {
    ...
    // Browser hints are never authoritative: category, priority and escalation are recomputed here.
    ...
    const validation = validatePublicContact({ requestType: "support", locale: payload.locale, name, email, phone: "", organization: "", stationId: context.stationId ?? normalizeStation(payload.stationId) ?? "", message: structuredMessage, website: "" });
    if (!validation.ok) return response(origin, { ok: false, error: validation.code }, 400);

    const { data, error } = await db.from("public_contact_requests").insert({ ...validation.value, ip_hash: ipHash }).select("id").single();
  ...
  if (action !== "legacy_contact") return response(origin, { ok: false, error: "UNKNOWN_ACTION" }, 400);

  // Backward-compatible path for existing public support and partner forms.

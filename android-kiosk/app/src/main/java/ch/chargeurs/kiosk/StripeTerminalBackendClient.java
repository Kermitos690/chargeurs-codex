...
/**
 * Minimal TEST-only transport to Agent 2's canonical Stripe Terminal backend.
 *
 * Secrets returned by Stripe (ConnectionToken / PaymentIntent client secret)
 * are kept only in method-local result objects and are never logged, persisted
 * or exposed to the WebView bridge.
 */
final class StripeTerminalBackendClient {
    ...
    /**
     * Explicit nested classes are used instead of Java records here because the
     * physical Android 11/12 kiosk build proved that the record companion class
     * could be referenced by D8-generated bytecode while missing at runtime.
     * Keep these simple classes packaged as ordinary DEX classes.
     */

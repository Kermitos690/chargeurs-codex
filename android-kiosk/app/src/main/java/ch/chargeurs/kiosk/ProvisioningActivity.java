...
public final class ProvisioningActivity extends Activity {
    ...
    private ScrollView buildView() {
        ...
        if (BuildConfig.DEBUG) {
            ...
            TextView tokenTitle = text("Token automatique — phase de test", 16, Color.WHITE);
            tokenTitle.setGravity(Gravity.CENTER);
            tokenCard.addView(tokenTitle, matchWrap(0, dp(8)));

            TextView tokenHelp = text(
                "Ce token est créé sur la borne. Copiez-le directement depuis cet écran. "
                    + "Un code d’appairage valide reste obligatoire.",
                13,
    ...
    private void copyTestToken() {
        ...
        Toast.makeText(this, "Token copié. Vous pouvez maintenant le coller.", Toast.LENGTH_LONG).show();
    }
    ...
    private void provision() {
        ...
        if (BuildConfig.DEBUG && !TestKioskToken.isValid(requestedTestToken)) {
            generateTestToken();
        }

        activateButton.setEnabled(false);
        activateButton.setText(R.string.enrollment_in_progress);
        final String proposedToken = BuildConfig.DEBUG ? requestedTestToken : null;
        executor.execute(() -> {
            try {
    ...
    private String enrollmentErrorMessage(Exception error) {
        ...
        switch (code) {
            ...
            case "TEST_TOKEN_NOT_ALLOWED":
            case "TEST_TOKEN_NOT_ACCEPTED":
                return "Le serveur de test n’a pas accepté le token créé sur cette tablette.";
            default:

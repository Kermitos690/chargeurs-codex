...
import { Clock3, HelpCircle, Loader2, RefreshCw, ShieldCheck, UserRound, X, Zap } from "lucide-react";
...
const COPY: Record<"fr" | "en" | "de", Copy> = {
  fr: {
    ...
    memberTitle: "Scannez avec",
    memberTitleAccent: "votre téléphone",
    memberPrivacy: "Connexion temporaire et sécurisée. Aucune donnée personnelle n’est saisie sur la borne.",
    memberRateLabel: "Tarif Client Chargeurs",
    memberScan: "Ouvrez l’appareil photo de votre téléphone et scannez le QR code.",
    memberError: "Connexion temporairement indisponible",
    retry: "Réessayer",
    eyebrow: "POWER WHEN YOU NEED IT",
    line1: "PLUS DE",
    line2: "BATTERIE ?",
    accent: "RÉGLÉ.",
    perHour: "/ heure",
    ...
    expressBody: "Sans compte. Choisissez une batterie, payez sur votre téléphone et partez.",
    clientKicker: "CLIENT",
    clientTitle: "CHARGEURS",
    clientBody: "Connectez-vous par QR et profitez de votre tarif Client Chargeurs.",
    cabinetTitle: "Batteries disponibles",
    cabinetSub: "Sélection automatique de la meilleure batterie",
  ...
  en: {
    ...
    memberTitle: "Scan with",
    memberTitleAccent: "your phone",
    memberPrivacy: "Temporary, secure connection. No personal data is entered on the station.",
    memberRateLabel: "Chargeurs member rate",
    memberScan: "Open your phone camera and scan the QR code.",
    ...
    expressTitle: "EXPRESS",
    expressBody: "No account. Choose a battery, pay on your phone and go.",
    clientKicker: "CHARGEURS",
    clientTitle: "MEMBER",
    clientBody: "Connect by QR and use your Chargeurs member rate.",
    cabinetTitle: "Batteries available",
    cabinetSub: "Best battery selected automatically",
  ...
  de: {
    ...
    memberTitle: "Scanne mit",
    memberTitleAccent: "deinem Smartphone",
    memberPrivacy: "Temporäre, sichere Verbindung. Auf der Station werden keine persönlichen Daten eingegeben.",
    memberRateLabel: "Chargeurs-Kundentarif",
    memberScan: "Öffne die Kamera deines Smartphones und scanne den QR-Code.",
    ...
    expressKicker: "MIETE",
    expressTitle: "EXPRESS",
    expressBody: "Ohne Konto. Batterie wählen, am Smartphone bezahlen und los.",
    clientKicker: "CHARGEURS",
    clientTitle: "KUNDE",
    clientBody: "Per QR verbinden und den Chargeurs-Kundentarif nutzen.",
    cabinetTitle: "Verfügbare Batterien",
    cabinetSub: "Beste Batterie wird automatisch gewählt",
...
export default function KioskPremiumGateV2() {
  ...
  useEffect(() => {
    if (timerActive && seconds === 0) returnHome();
  }, [seconds, timerActive, returnHome]);
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      document.documentElement.dataset.kioskJourney = "client";
      setStage("connected");
      window.setTimeout(() => setStage("guest"), 1400);
    };
  ...
  const journeyControl = timerActive ? (
    <div className="ck2-journey-control" role="status" aria-live="polite">
      ...
      <button type="button" onClick={returnHome} aria-label={copy.cancel}>
  ...
  if (checking) {
    return (
      <div className="ck2-shell ck2-loading">
  ...
  if (stage === "connected") {
    return (
      <div className="ck2-shell ck2-connected">
        {journeyControl}
        <div className="ck2-connected-card">
          <div className="ck2-user-ring"><UserRound /></div>
          <BrandLogo size="md" />
      ...
      <div className="ck2-shell ck2-member">
        ...
        <header className="ck2-topbar">
          ...
          <div className="ck2-top-actions">
            <LanguageSwitcher />
            <button type="button" className="ck2-pill" onClick={returnHome}>
              <X /> {copy.cancel}
            </button>
          </div>
        </header>
        ...
        <main className="ck2-member-grid">
          <section className="ck2-member-copy">
            ...
            <div className="ck2-member-rate">
              {money(options?.member?.hourly_cents, options?.member?.currency)}
              <small>{copy.perHour}</small>
            </div>
            <div className="ck2-security"><ShieldCheck /> {copy.secure}</div>
          </section>

          <section className="ck2-qr-card">
            <div className="ck2-qr-head">
              <BrandLogo size="sm" />
              <span>{copy.memberRateLabel}</span>
            </div>
            {connectUrl && !pairingError ? (
              <div className="ck2-qr-wrap">
                <QRCodeSVG value={connectUrl} size={330} level="M" bgColor="#ffffff" fgColor="#06090f" marginSize={2} />
              </div>
            ) : pairingError ? (
              <div className="ck2-member-error">
                <p>{copy.memberError}</p>
                <button type="button" onClick={() => void startMember()}>{copy.retry}</button>
              </div>
            ) : (
              <Loader2 className="ck2-spin" />
            )}
  ...
  return (
    <div className="ck2-shell ck2-home">
      ...
      <header className="ck2-topbar">
        <BrandLogo size="md" />
        <div className="ck2-top-actions">
          <button type="button" className="ck2-pill" onClick={() => void refreshOptions()} disabled={refreshing}>
            <RefreshCw className={refreshing ? "ck2-spin-small" : ""} /> {copy.refresh}
          </button>
          <button type="button" className="ck2-pill" onClick={() => window.dispatchEvent(new CustomEvent("chargeurs:open-kiosk-help"))}>
            <HelpCircle /> {copy.help}
          </button>
      ...
      <main className="ck2-home-grid">
        <section className="ck2-hero-copy">
          ...
          <div className="ck2-choice-grid">
            <button type="button" className="ck2-choice ck2-choice-express" onClick={chooseGuest} disabled={!options?.guest}>
        ...
        <section className="ck2-device-stage" aria-label="Chargeurs.ch">
          ...
          <div className="ck2-device">
            <div className="ck2-device-screen">
              <BrandLogo size="sm" />
              <strong>{copy.cabinetTitle}</strong>
              <span>{copy.cabinetSub}</span>
            </div>
            <div className="ck2-device-divider" />
            <div className="ck2-device-brand"><BrandLogo size="sm" /></div>
            <div className="ck2-device-slots">
              {[1, 2, 3, 4].map((slot) => (
                <div className={`ck2-device-slot ${slot === 1 || slot === 3 ? "is-ready" : ""}`} key={slot}>
                  <span />
                  <i>{slot}</i>
                </div>
              ))}

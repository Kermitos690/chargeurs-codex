...
function Conditions() {
  return <>
    ...
    <p>Le tarif public de référence est de {formatChf(PUBLIC_PRICING.hourlyRate)} par heure, facturé par tranches de {PUBLIC_PRICING.incrementMinutes} minutes, avec un plafond de {formatChf(PUBLIC_PRICING.dailyCap)} par jour. Le profil tarifaire applicable est figé côté serveur au début de la location. Le montant définitif est calculé à partir de cette configuration après la détection du retour ; l'écran du téléphone ou de la borne n'est jamais autorisé à inventer ou modifier le prix.</p>

    <h2>3. Garantie de 30 CHF — carte et portefeuilles compatibles</h2>
    <p>Lorsque le client choisit une carte bancaire ou un portefeuille utilisant les rails carte et éligible auprès de Stripe (par exemple Apple Pay ou Google Pay), une autorisation temporaire de {formatChf(PUBLIC_PRICING.deposit)} est demandée à l'établissement bancaire. Ce montant n'est pas destiné à être encaissé intégralement au départ : après le retour, Chargeurs.ch demande la capture du montant réellement dû et la libération du solde de l'autorisation. Selon la banque, l'autorisation peut apparaître temporairement comme une opération en attente ou un montant indisponible. Les délais d'affichage et de libération dépendent de l'établissement financier.</p>

    <h2>4. Garantie de 30 CHF — TWINT</h2>
    <p>TWINT ne suit pas le même mécanisme qu'une autorisation carte dans ce parcours. Lorsque TWINT est choisi, {formatChf(PUBLIC_PRICING.deposit)} sont débités au début. Après le retour et le calcul du prix définitif, la partie non due est automatiquement remboursée par Stripe. Le délai d'apparition du remboursement dépend de TWINT, de la banque et des circuits de paiement.</p>

    <h2>5. Moyen de paiement enregistré et montants complémentaires</h2>
    <p>Dans le parcours carte, le client autorise Stripe à enregistrer de manière sécurisée le moyen de paiement pour permettre à Chargeurs.ch de tenter ultérieurement le règlement de montants contractuellement dus liés à cette même location, notamment si le prix final dépasse la garantie disponible ou en cas de non-retour. Chargeurs.ch ne reçoit ni ne stocke le numéro complet de carte. Un débit complémentaire peut nécessiter une nouvelle authentification et n'est jamais considéré comme garanti simplement parce qu'une autorisation initiale de {formatChf(PUBLIC_PRICING.deposit)} a été accordée. Si un règlement complémentaire ne peut pas être exécuté automatiquement, Chargeurs.ch peut demander sa régularisation par un autre moyen de paiement.</p>
    ...
    <p>En cas de non-retour dans le délai applicable, de perte ou de sortie définitive de la batterie du réseau, le montant total dû peut atteindre {formatChf(PUBLIC_PRICING.nonReturnTotal)} selon les circonstances et les conditions affichées lors de la location. La garantie initiale de {formatChf(PUBLIC_PRICING.deposit)} est imputée sur ce total : elle ne constitue pas, à elle seule, une garantie bancaire de {formatChf(PUBLIC_PRICING.nonReturnTotal)}. Le solde pouvant rester dû est au maximum de {formatChf(PUBLIC_PRICING.nonReturnBalanceAfterDeposit)} dans la configuration tarifaire actuelle. Chargeurs.ch peut tenter de percevoir ce solde sur un moyen de paiement enregistré lorsque le droit applicable, Stripe et les exigences d'authentification le permettent, ou demander une régularisation séparée.</p>

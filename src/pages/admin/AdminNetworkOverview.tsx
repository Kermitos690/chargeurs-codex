...
export default function AdminNetworkOverview() {
  ...
  const load = useCallback(async (quiet = false) => {
    ...
    if (error || !result?.ok) {
      setLoadError(result?.error ?? error?.message ?? "La vue réseau détaillée n’a pas pu être chargée.");
  ...
  return (
    <div className="animate-fade-in space-y-6 pb-12">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <div className="mb-2 text-xs font-bold uppercase tracking-[.18em] text-primary">Product Command Center · niveau 2</div>
          <h1 className="font-display text-3xl font-bold">Vue réseau détaillée</h1>
          <p className="mt-2 max-w-3xl text-sm text-muted-foreground">Métriques, tendances et alertes complètes issues des données réelles Chargeurs + fournisseur.</p>
        </div>
      ...
      {data && metrics && <>
        ...
        <section className="grid gap-6 xl:grid-cols-2">
          <div className="glass liquid-border rounded-2xl p-5 sm:p-6"><div className="mb-5"><h2 className="font-display text-xl font-bold">Activité réseau · 7 jours</h2><p className="mt-1 text-sm text-muted-foreground">Locations créées et paiements enregistrés, sans données de démonstration.</p></div><ActivityChart rows={trendData} /></div>
          <div className="glass liquid-border rounded-2xl p-5 sm:p-6"><div><h2 className="font-display text-xl font-bold">Revenu encaissé · 7 jours</h2><p className="mt-1 text-sm text-muted-foreground">Montants capturés moins remboursements, en CHF.</p></div><RevenueChart rows={trendData} /></div>
        </section>

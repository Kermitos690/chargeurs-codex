...
export function CommandCenterMobileShell({ model, roles, generatedAt, loading, error, onRefresh }: Props) {
  ...
  return (
    <div className="relative min-h-[calc(100dvh-4rem)] bg-background/90 pb-28">
      <main className="mx-auto max-w-2xl space-y-4 px-4 py-5">
        ...
        {tab === "home" && <>
          <div className="flex items-start justify-between gap-3">
            ...
            <Button variant="ghost" size="icon" aria-label="Actualiser le Product Command Center" onClick={onRefresh} disabled={loading} className="shrink-0">
              <RefreshCw className={`h-4 w-4 ${loading ? "animate-spin" : ""}`} />
            </Button>
          </div>
          <p className="text-[11px] text-muted-foreground">Données mises à jour {relative(generatedAt)}</p>
        ...
        {tab === "more" && <><div><h1 className="font-display text-2xl font-bold">Plus</h1><p className="mt-1 text-sm text-muted-foreground">Accéder aux analyses et outils spécialisés. Le menu principal reste accessible en haut de l’écran.</p></div><MorePanel roles={roles} /></>}
      </main>

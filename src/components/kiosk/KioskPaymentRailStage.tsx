...
const COPY = {
  fr: { eyebrow:"PAIEMENT SÉCURISÉ", ready:"Comment souhaitez-vous payer ?", terminal:"Sans contact", terminalSub:"Présentez votre carte ou votre téléphone sur le terminal", qr:"QR code", qrSub:"Scannez et payez sur votre téléphone", qrOnly:"Paiement par QR code", qrOnlySub:"Préparation du paiement sur votre téléphone…", processing:"Paiement sans contact en cours", processingSub:"Suivez les instructions affichées sur le terminal.", reconnecting:"Connexion au lecteur…", retry:"Réessayer le lecteur", locked:"Méthode de paiement engagée" },
  en: { eyebrow:"SECURE PAYMENT", ready:"How would you like to pay?", terminal:"Contactless", terminalSub:"Tap your card or phone on the payment reader", qr:"QR code", qrSub:"Scan and pay on your phone", qrOnly:"Pay by QR code", qrOnlySub:"Preparing payment on your phone…", processing:"Contactless payment in progress", processingSub:"Follow the instructions shown on the payment reader.", reconnecting:"Connecting reader…", retry:"Retry reader", locked:"Payment method engaged" },
  de: { eyebrow:"SICHERE ZAHLUNG", ready:"Wie möchten Sie bezahlen?", terminal:"Kontaktlos", terminalSub:"Karte oder Smartphone an das Terminal halten", qr:"QR-Code", qrSub:"Scannen und auf dem Smartphone bezahlen", qrOnly:"Per QR-Code bezahlen", qrOnlySub:"Zahlung auf dem Smartphone wird vorbereitet…", processing:"Kontaktlose Zahlung läuft", processingSub:"Folgen Sie den Anweisungen auf dem Terminal.", reconnecting:"Leser wird verbunden…", retry:"Leser erneut verbinden", locked:"Zahlungsart aktiv" },
} as const;

function parseProjection(raw: string | undefined): NativeReaderProjection | null {
  if (!raw) return null;
  try { const parsed = JSON.parse(raw) as NativeReaderProjection; return parsed && typeof parsed === "object" ? parsed : null; }
  catch { return null; }
}

export function KioskPaymentRailStage(props: Props) {
  const { lang, rentalSessionId, stationId, stationOnline, selectedSlot, pricingReady, pricingCurrency, inProgress=false, onChooseQr, onTerminalEngaged, onServerConfirmed } = props;
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    const refresh = () => { const next = parseProjection(native?.getPaymentReaderStatus?.()); if (!cancelled && next) setReader(next); };
    refresh();
  ...
  const model = useMemo(() => buildChargeursPresentationModel({
    width: window.innerWidth, height: window.innerHeight, nativeBridge,
    reducedMotion: window.matchMedia?.("(prefers-reduced-motion: reduce)").matches ?? false,
    journeyState: inProgress ? "PAYMENT_IN_PROGRESS" : "PAYMENT_READY",
    stationId, stationOnline, selectedSlot, pricingReady, pricingCurrency, reader, localRail, localRailState,
  ...
  /* No usable READY reader = no fake choice screen. Open the QR rail directly. */
  useEffect(() => {
    if (inProgress || model.reader.capability !== "QR_ONLY" || !model.payment.canChooseQr || qrAutoStartedRef.current) return;
    qrAutoStartedRef.current = true;
    railTapLockRef.current = true;
    setLocalRail("QR");
    setLocalRailState("CLAIMING");
    onChooseQr();
  }, [inProgress, model.reader.capability, model.payment.canChooseQr, onChooseQr]);
  ...
  const chooseTerminal = () => {
    ...
    } catch { setNativeError("TERMINAL_START_FAILED"); }
    if (!accepted) {
  ...
  const retryReader = () => { setNativeError(null); const next = parseProjection(native?.refreshPaymentReader?.()); if (next) setReader(next); };

  if (inProgress || model.payment.rail === "TERMINAL") {
    return <div className="kiosk-payment-rail-stage flex w-full max-w-5xl flex-col items-center gap-7 px-5 text-center" data-payment-rail="TERMINAL" data-reader-state={model.reader.state}>
      ...
      <div className="inline-flex items-center gap-3 rounded-full border border-white/10 bg-white/5 px-5 py-3 text-base font-bold"><Loader2 className="h-5 w-5 animate-spin text-primary" /><span>{model.payment.serverConfirmed ? "SERVER CONFIRMED" : `${model.reader.state} · ${model.payment.railState}`}</span></div>
    </div>;
  }

  if (model.reader.capability === "QR_ONLY") {
    return <div className="kiosk-payment-rail-stage flex w-full max-w-5xl flex-col items-center gap-7 px-5 text-center" data-payment-capability="QR_ONLY" data-reader-state={model.reader.state}>
      ...
      {nativeBridge && (model.reader.state === "ERROR" || model.reader.state === "ABSENT") && <Button variant="ghost" onClick={retryReader} className="h-12 gap-2 rounded-full px-6"><RefreshCw className="h-4 w-4" />{copy.retry}</Button>}
      {nativeError && <p className="text-sm font-semibold text-warning">{nativeError}</p>}
    </div>;
  }

  return <div className="kiosk-payment-rail-stage flex w-full max-w-6xl flex-col items-center gap-7 px-5 text-center" data-payment-capability="TERMINAL_AND_QR" data-reader-state={model.reader.state}>
    <div className="inline-flex items-center gap-2 rounded-full border border-cyan-200/20 bg-cyan-300/10 px-4 py-2 text-sm font-black tracking-[.14em] text-cyan-100"><ShieldCheck className="h-4 w-4" />{copy.eyebrow}</div>

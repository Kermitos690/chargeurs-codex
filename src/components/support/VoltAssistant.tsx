...
type Triage = { category: VoltCategory; priority: VoltPriority; escalate: boolean; reply: string; provider: "deterministic"; externalCall: false };
...
export function VoltAssistant({ mode, userName = "", userEmail = "", stationId = "", rentalId = "", contextHint: _contextHint = "", onCaseCreated }: Props) {
  ...
  const createCase = async (text: string, triage: Triage) => {
    ...
    if (invokeError || !data?.ok) {
      ...
      setError(code === "RATE_LIMITED" ? "Trop de demandes ont été envoyées depuis cette connexion. Réessayez plus tard." : code === "RENTAL_NOT_ACCESSIBLE" ? "La référence de location ne peut pas être utilisée avec ce compte." : "Le dossier n'a pas pu être enregistré. Vous pouvez réessayer sans ressaisir votre message.");
      setPendingCase({ text, triage });
      return;
    }
  ...
  const handleUserMessage = async (text: string) => {
    ...
    const { data, error: invokeError } = await supabase.functions.invoke("public-contact", {
      body: { action: "volt_triage", mode, message: clean, stationId, rentalId, locale: lang },
    });
    setBusy(false);
    if (invokeError || !data?.ok || !data?.triage) {
      setError(data?.error === "RENTAL_NOT_ACCESSIBLE" ? "La référence de location ne peut pas être utilisée avec ce compte." : "Volt n'arrive pas à analyser ce message pour le moment. Réessayez sans transmettre de données de carte.");
      addVoltMessage("Je n'arrive pas à vérifier votre demande pour le moment. Aucune donnée de paiement sensible ne doit être envoyée dans la conversation.");
      return;
    }
    const triage = data.triage as Triage;
  ...
  return (
    <section className="glass-strong liquid-border overflow-hidden rounded-3xl" aria-label="Volt, assistant Chargeurs.ch">
      ...
      <div className="border-t border-border/70 bg-card/30 px-5 py-5 sm:px-7">
        ...
        <div className="mt-3 flex items-start gap-2 text-[0.72rem] leading-5 text-muted-foreground"><Sparkles className="mt-0.5 h-3.5 w-3.5 shrink-0" /><p>Volt reste limité à Chargeurs.ch. Le triage et les autorisations sont recalculés côté serveur. Aucun fournisseur IA externe n'est actif dans cette version.</p></div>
      </div>
    </section>
  );
}

...
const QUICK_ACTIONS = [
  ...
  "Découvrir l'offre de lancement",
...
const QUICK_PROMPTS: Record<string, string> = {
  ...
  "Découvrir l'offre de lancement": "Je veux découvrir l'offre de lancement Chargeurs.ch.",
...
function loadingLabel(raw: string) {
  ...
  if (/(prix|tarif|combien|co[ûu]t|palier|offre|lancement)/.test(text)) return "Volt vérifie les offres…";
...
function fallbackTriage(raw: string): Triage {
  ...
  if (/(prix|tarif|combien|co[ûu]t|offre|lancement)/.test(text)) return { category: "pricing", priority: "normal", escalate: false };
...
function suggestionsFor(message: string, triage: Triage): string[] {
  ...
  if (triage.category === "payment" && !triage.escalate) return ["J'ai eu un débit inattendu", "Mon paiement a été refusé", "J'attends un remboursement", "J'ai payé mais la batterie ne sort pas"];
  if (triage.category === "ejection") return ["Le paiement est confirmé", "Je vois un débit mais rien ne sort", "Je veux contacter le support"];
  if (triage.category === "return") return ["La location continue après le retour", "Je ne vois pas le retour dans mon compte", "Je veux contacter le support"];
  if (triage.category === "station") return ["L'écran ne répond plus", "Un emplacement semble bloqué", "Je veux signaler cette borne"];
  if (triage.category === "pricing") return ["Combien coûte 2 heures ?", "Combien coûte 24 heures ?", "Je veux découvrir l'offre de lancement", "Et pour les clients Chargeurs+ ?"];
  if (triage.category === "account") return ["Comment fonctionne Chargeurs+ ?", "Je veux découvrir l'offre de lancement", "Où voir mon crédit ?", "Comment retrouver ma location ?"];
  if (/(salut|bonjour|ça va|ca va|yo|yoyo)/.test(text)) return ["Quels sont les tarifs ?", "Je veux découvrir l'offre de lancement", "Comment louer une batterie ?", "J'ai besoin d'aide"];
  return ["Je veux découvrir l'offre de lancement"];
}
...
async function invokeVoltChat(body: Record<string, unknown>) {
  ...
  try {
    const response = await fetch("/api/volt-chat", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body), credentials: "same-origin", cache: "no-store" });
...
async function invokeVoltCase(body: Record<string, unknown>, mode: VoltMode) {
  ...
  try {
    const response = await fetch("/api/volt-support", { method: "POST", headers, body: JSON.stringify(body), credentials: "same-origin", cache: "no-store" });
...
export function VoltAssistantV2({ mode, userName = "", userEmail = "", stationId = "", rentalId = "", contextHint = "", onCaseCreated }: Props) {
  ...
  const [messages, setMessages] = useState<VoltMessage[]>([{ id: id("volt"), role: "volt", text: mode === "client" ? "Bonjour, je suis Volt. Posez-moi votre question." : initialPublicEmailReady ? "Bonjour, je suis Volt. Comment puis-je vous aider ?" : "Bonjour, je suis Volt ⚡ Avant de commencer, indiquez votre adresse email. Elle servira à retrouver cette conversation si un suivi devient nécessaire." }]);
  ...
  const conversationHistory = useMemo(() => messages.slice(-10).map((message) => ({ role: message.role === "volt" ? "assistant" : "user", content: message.text })), [messages]);

  const createCase = async (text: string, triage: Triage) => {
    if (busy || caseId) return;
    if (!supportIdentityReady) { setPendingCase({ text, triage }); return; }
    setLoadingText("Volt transmet le dossier…"); setBusy(true); setError(null);
    ...
    if (invokeError || !data?.ok) {
      ...
      setPendingCase({ text, triage }); return;
    }
    const requestId = String(data.requestId);
    setCaseId(requestId); setPendingCase(null); setSuggestions([]);
  ...
  const handleUserMessage = async (text: string) => {
    ...
    setLoadingText(loadingLabel(clean)); setBusy(true);
    const { data: chat, error: chatError } = await invokeVoltChat({ mode, message: clean, history, stationId, rentalId, contextHint: mode === "client" ? contextHint : "", locale: lang });
    setBusy(false);
    const fallback = fallbackTriage(clean);
    const triage: Triage = { category: chat?.triage?.category ?? fallback.category, priority: chat?.triage?.priority ?? fallback.priority, escalate: chat?.triage?.escalate ?? fallback.escalate };
    const reply = !chatError && chat?.ok && chat.reply ? chat.reply : "Je n'ai pas pu obtenir la réponse complète. Vous pouvez reformuler ou choisir une option ci-dessous.";
  ...
  useEffect(() => {
    const onPrompt = (event: Event) => {
      ...
      if (!prompt) return;
      if (mode === "public" && !emailReady) { setPendingPrompt(prompt); return; }
      void handleUserMessage(prompt);
    };
    window.addEventListener("volt:prompt", onPrompt);
    return () => window.removeEventListener("volt:prompt", onPrompt);
  });

  useEffect(() => {
    if (!emailReady || !pendingPrompt || busy) return;
    const prompt = pendingPrompt;
    setPendingPrompt("");
    const timer = window.setTimeout(() => void handleUserMessage(prompt), 80);
    return () => window.clearTimeout(timer);
  }, [emailReady, pendingPrompt, busy]);

  const submitEmail = (event: FormEvent) => {
    event.preventDefault();
    if (!validEmail(identity.email)) { setError("Saisissez une adresse email valide pour commencer."); return; }
    setError(null); setEmailReady(true);
    addVoltMessage("Parfait, merci. Comment puis-je vous aider ?");
    if (!pendingPrompt) setSuggestions(["Quels sont les tarifs ?", "Je veux découvrir l'offre de lancement", "Comment louer une batterie ?", "J'ai besoin d'aide"]);
  };
  ...
  const submitIdentity = (event: FormEvent) => {
    ...
    if (identity.name.trim().length < 2) { setError("Indiquez votre nom pour transmettre ce dossier."); return; }
  ...
  return (
    <section className="glass-strong liquid-border overflow-hidden rounded-3xl" aria-label="Volt, assistant Chargeurs.ch">
      <div className="max-h-[32rem] space-y-4 overflow-y-auto px-5 py-6 sm:px-7" aria-live="polite">
        ...
        {needsPublicEmail && <form onSubmit={submitEmail} className="rounded-2xl border border-primary/25 bg-primary/5 p-4">
          ...
          {pendingPrompt && <p className="mt-2 rounded-xl bg-background/60 px-3 py-2 text-xs text-foreground">Après validation, Volt reprendra : « {pendingPrompt} »</p>}
          <div className="mt-3 flex gap-2"><Input type="email" inputMode="email" autoComplete="email" placeholder="vous@exemple.ch" value={identity.email} onChange={(event) => setIdentity((current) => ({ ...current, email: event.target.value }))} aria-label="Votre email" required /><Button type="submit" className="shrink-0 rounded-xl bg-gradient-primary font-bold">Continuer</Button></div>
          {error && <p role="alert" className="mt-2 text-xs text-destructive">{error}</p>}
        </form>}

        {!needsPublicEmail && suggestions.length > 0 && !busy && !caseId && <div className="flex flex-wrap gap-2" aria-label="Réponses proposées">{suggestions.map((suggestion) => <button key={suggestion} type="button" onClick={() => void handleUserMessage(suggestion)} className="rounded-full border border-primary/30 bg-primary/10 px-3 py-2 text-left text-xs font-semibold text-foreground transition hover:border-primary/60 hover:bg-primary/15">{suggestion}</button>)}</div>}

        {pendingCase && !caseId && !needsPublicEmail && (mode === "public" ? <form onSubmit={submitIdentity} className="rounded-2xl border border-primary/25 bg-primary/5 p-4">
          <div className="flex items-start gap-2"><AlertTriangle className="mt-0.5 h-4 w-4 shrink-0 text-primary" /><div><p className="text-sm font-medium">Une vérification support peut être utile</p><p className="mt-1 text-xs text-muted-foreground">Votre email est déjà enregistré pour cette conversation. Indiquez simplement votre nom si vous souhaitez transmettre le dossier.</p></div></div>
          <div className="mt-3"><Input aria-label="Votre nom" placeholder="Votre nom" required minLength={2} maxLength={120} value={identity.name} onChange={(event) => setIdentity((current) => ({ ...current, name: event.target.value }))} /></div>
          {error && <p role="alert" className="mt-2 text-xs text-destructive">{error}</p>}
          <div className="mt-3 flex flex-wrap gap-2"><Button type="submit" disabled={busy} className="rounded-full bg-gradient-primary font-bold"><Send className="mr-2 h-4 w-4" />Transmettre au support</Button><Button type="button" variant="ghost" className="rounded-full" onClick={() => { setPendingCase(null); setError(null); }}>Pas maintenant</Button></div>
        </form> : <div className="rounded-2xl border border-primary/25 bg-primary/5 p-4"><div className="flex items-start gap-2"><AlertTriangle className="mt-0.5 h-4 w-4 shrink-0 text-primary" /><div><p className="text-sm font-medium">Une vérification support peut être utile</p><p className="mt-1 text-xs text-muted-foreground">Rien n'est envoyé sans votre confirmation.</p></div></div><div className="mt-3 flex flex-wrap gap-2"><Button type="button" disabled={busy} className="rounded-full bg-gradient-primary font-bold" onClick={() => void createCase(pendingCase.text, pendingCase.triage)}><Send className="mr-2 h-4 w-4" />Transmettre au support</Button><Button type="button" variant="ghost" className="rounded-full" onClick={() => { setPendingCase(null); setError(null); }}>Pas maintenant</Button></div></div>)}
      ...
      {!needsPublicEmail && <div className="border-t border-border/70 bg-card/30 px-5 py-5 sm:px-7">
        {!caseId && suggestions.length === 0 && <div className="mb-4 flex flex-wrap gap-2">{QUICK_ACTIONS.map((label) => <button key={label} type="button" disabled={busy} onClick={() => void handleUserMessage(QUICK_PROMPTS[label])} className="rounded-full border border-border bg-background/70 px-3 py-2 text-xs font-semibold text-muted-foreground transition hover:border-primary/40 hover:text-foreground disabled:opacity-50">{label}</button>)}</div>}
